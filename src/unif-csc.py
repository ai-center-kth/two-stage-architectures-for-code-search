...
def test(dataset, code_model, desc_model, dot_model, results_path):
    print("Testing model...")
    # Hardcoded
    # size of the dataset = 22176
    code_test_ds_np = iter(dataset.batch(22176)).get_next()[0][0].numpy()
    code_test_ds_np = code_test_ds_np.reshape((22176,-1))

    desc_test_ds_np = iter(dataset.batch(22176)).get_next()[0][1].numpy()
    desc_test_ds_np = desc_test_ds_np.reshape((22176,-1))

    code_embeddings = []
    desc_embeddings = []

    print("Embedding code and descriptions...")
    for i in range(0,22176):
